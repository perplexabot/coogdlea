...
# Example 2:
    # Given nums = [0,0,1,1,1,2,2,3,3,4], Your function should return length = 5, with 
...
class Solution:
    def removeDuplicates(self, nums):
        ...
        foundInd = 1
        curr = 0
        while nums[-1] != nums[curr]:
            foundInd = findFirstNotFrom(nums[curr], foundInd)
            if foundInd is None:
                return curr+1
            nums[curr+1] = nums[foundInd]
            curr += 1
        return curr + 1


S = Solution()
tries = [ [1,1,2], [0,0,1,1,1,2,2,3,3,4], [1]*10, [], [1] ]
for t in tries:
    print( "test: ", str(t) )
    ans = S.removeDuplicates( t )
    print( ''.join( ["ans: ", str(ans)] ) )
    print( "testAfter: ", str(t) )
    print( "--------------" )
