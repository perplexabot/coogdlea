...
A:
    ...
    can list be len 1?  no
    given list of len 2?    if same elem - return 0 else return diff
...
for (colors, exp) in cases:
    assert (
        got := sol.maxDistance(colors)
    ) == exp, f"Failed case ({colors}) - expecting ({exp}), got ({got})."
