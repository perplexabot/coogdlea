...
There are n seats and n students in a room. You are given an array seats of length n, where seats[i]
    is the position of the ith seat. You are also given the array students of length n, where
...
A:
    seats != students   ->  not possible according to constraint
    empty lists         ->  not possible according to constraint
    negative positions  ->  not position according to constraint
...
for (seats, students, exp) in cases:
    assert (
        got := sol.minMovesToSeat(seats, students)
    ) == exp, f"Failed case ({seats}, {students}) - expecting ({exp}), got ({got})"
