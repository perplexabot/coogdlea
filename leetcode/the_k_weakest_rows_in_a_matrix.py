...
D:
    ...
    sort by sol first, then by row, weakest to strongest:
        0, 2, 3, 4
    return [0,2]

P:
    create list of tuples (sol_cnt, row)
    get mins by sol_cnt
    get mins by row
    return
"""
        ...
        for ind, row in enumerate(mat):
            ...
            for elem in row:
                if not elem:
                    break
                sol_cnt += 1
            strengths.append((sol_cnt, ind))
        strengths.sort()
        return [x[1] for x in strengths[:k]]
