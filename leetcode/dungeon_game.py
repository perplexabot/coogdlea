        ...
        for i in range(len(dungeon) - 1, -1, -1):
            for j in range(len(dungeon[0]) - 1, -1, -1):
                if i + 1 >= len(dungeon) and j + 1 >= len(dungeon[0]):
                    min_hp[(i, j)] = max(0, 0 - dungeon[i][j])
                elif i + 1 >= len(dungeon):
                    min_hp[(i, j)] = max(0, min_hp[(i, j + 1)] - dungeon[i][j])
                elif j + 1 >= len(dungeon[0]):
                    min_hp[(i, j)] = max(0, min_hp[(i + 1, j)] - dungeon[i][j])
                else:
                    min_hp[(i, j)] = max(
                        0, min(min_hp[(i + 1, j)], min_hp[(i, j + 1)]) - dungeon[i][j]
                    )
        print(min_hp)
        return min_hp[(0, 0)] + 1
...
sol = Solution()
for dungeon, exp in inps:
    print(f"Finding min HP for dungeon {dungeon} and asserting...")
